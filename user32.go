...
func FindWindow(className, winTitle string) HWND {
	lpClassName, err := syscall.UTF16PtrFromString(className)
	if err != nil {
		return 0
	}

	lpWindowName, err := syscall.UTF16PtrFromString(winTitle)
	if err != nil {
		return 0
	}
...
func FindAllWindows() []HWND {
	hwndArr := []HWND{}
	cb := syscall.NewCallback(func(h syscall.Handle, p uintptr) uintptr {
		hwndArr = append(hwndArr, HWND(h))
		return 1
	})
	EnumWindows(cb, 0)
	return hwndArr
...
func GetWindowText(hwnd HWND) string {
	str := make([]uint16, 200)
	r0, _, _ := syscall.Syscall(getWindowTextW.Addr(), 3, uintptr(hwnd), uintptr(unsafe.Pointer(&str[0])), uintptr(int32(len(str))))
	len := int32(r0)
	if len == 0 {
		return ""
	} else {
...
func SetWindowsText(hwnd HWND, text string) bool {
...
func GetClassName(hWnd HWND) string {
	...
	ret, _, _ := syscall.Syscall(getClassName.Addr(), 3,
		uintptr(hWnd),
		uintptr(unsafe.Pointer(&str[0])),
		uintptr(int32(len(str))))
